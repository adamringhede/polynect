...
/*server.on('uncaughtException', function (req, res, route, err) {
  ...
  console.error(err.stack);
  res.send(500, "not good")
});*/

//.well-known/acme-challenge/g6AjXaXX7Ilcv_zR6yaCXHEENYGwSquK7OzhciVS3QQ
server.get(/fq07RQmn4GqPu_maM9QZaJu8HsPb2D_o-L3Kdyu8Ygg/, (req, res, next) => {
  let body = "fq07RQmn4GqPu_maM9QZaJu8HsPb2D_o-L3Kdyu8Ygg.vRn-41SNGgZLDsvNIPqNkas-JyJ1zeEbZWKaD0VHodo";
  res.writeHead(200, {
...
server.get('/v1', function (req, res, next) {
  res.send('Happy matchmaking! If you have any questions, don\'t hesitate to email info@polynect.io. Check out the documentation at https://www.polynect.io');
  next();
});
...
server.use(function (req, res, next) {
  ...
  if (req.headers.authorization != null && req.headers.authorization.split(' ').length > 1) {
    authmethod = req.headers.authorization.split(' ')[0].toLowerCase()
  }
  if (authenticate) {
    if (authmethod == 'playfab') {
      console.log('Authenticating with PlayFab');
      // we can however only perform this if we have the game's titleId
      req.user = new Models.Account();
      req.user.role = 'playfab';
      next()
    } else {
