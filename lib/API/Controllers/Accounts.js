...
module.exports = function(route) {
  ...
  route.post('/v1/accounts/register',
    ...
    $.createAccessToken('account', Models.Client.DEV_PORTAL, 1200),
    function (req, res, next) {
      req.resources.account.developer_token = req.oauth.token
      req.resources.account.developer_token_expires = req.oauth.expires
      next();
    },
    function (req, res, next) {
    ...
    $.createAccessToken('account', Models.Client.DEV_PORTAL),
    /*function (req, res, next) {
      req.body.developer_token = req.oauth.token
      req.body.developer_token_expires = req.oauth.expires
      next();
    },
    $.update('account', {
      developer: 'developer_token developer_token_expires',
      admin: 'developer_token developer_token_expires'
    }),*/
    $.output('account', Views.Account.withToken)
  );
  ...
  route.post('/v1/accounts/access_token',
    $.restrict('admin developer'),
    function (req, res, next) {
      if (!req.resources) req.resources = {};
      req.resources.account = req.user;
      next();
    },
    $.createAccessToken('account', Models.Client.DEV_PORTAL, 1200),
    function (req, res, next) {
