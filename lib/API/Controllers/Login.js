...
function findOrCreatePlayerWithProvider (uid, provider, gameId, callback) {
  Models.Player.findOneAndUpdate(
    {"provider.uid": uid, "provider.alias": provider, "game": gameId},
...
module.exports = function (server) {
  server.post('/game/:game/login/:provider', function (req, res, next) {
  ...
  server.post('/game/:game/login', function (req, res, next) {
    Models.Player.findWithCredentials(req.body.username, req.body.password, req.params.game, function (err, model) {
      if (err) {
        res.send(500, 'Internal server error');
        console.error(err);
      } else {
