...
module.exports = function(server) {
  ...
  server.post('/games/:game/players', function(req, res, next) {
    getGame(req, res, next, function(game){
      ...
      Models.Account.createWithCredentials({username: credentials.username, password: credentials.password, game: req.params.game}
        , function(err, model) {
        if (err === Models.Account.ERROR_USERNAME_TAKEN) {
          res.send(409, 'Username is already taken')
        } else if (err) {
          res.send(500, 'Internal server error')
        } else if (model) {
          res.send(200, Views.Player(model))
        }
        return next();
      })
    });
