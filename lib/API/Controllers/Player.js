...
module.exports = function (server) {
  server.get('/games/:game/players/:player', function (req, res, next) {
    Models.Game.findOne({_id: req.params.game}, function (err, model) {
      if (err) { res.send(500, 'Could not retrieve game'); return next() }
      else if (!model) { res.send(404, 'Could not find game with id ' + req.params.game); return next() }
      Models.Player.findOne({_id: req.params.player, game: req.params.game}, function (err, model) {
        if (err) {
