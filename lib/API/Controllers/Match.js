...
const playFabAuth = (req, res, next) => {
  if (req.user.role == 'playfab') {
    if (req.resources.game == null || req.resources.game.playfab == null || req.resources.game.playfab.enabled != true) {
      return next(new restify.errors.BadRequestError("PlayFab needs to be enabled on the game."))
    }
    PlayFab.authRequestWithTicket(req, next)
  } else {
    next()
  }
};
...
module.exports = function(server) {
  ...
  function isCharacterNeeded (game) {
    for (let key of Object.keys(game.matchmaking_config.attributes)) {
      let attr = game.matchmaking_config.attributes[key];
  ...
  function createPlayfabRequest(req) {
    const sessionTicket = req.headers.authorization.split(' ')[1];
  ...
  server.get('/v1/matches/:match',
    $.restrict('admin developer player playfab'),
    $.load({match: 'Match'}),
    (req, res, next) => {
      req.params.game = req.resources.match.game.id;
      next()
    },
    $.load({game: 'Game'}),
    playFabAuth,
  ...
  server.post('/v1/matches/:match/players',
    ...
    (req, res, next) => {
      if (req.resources.match.size >= req.resources.match.max) {
        return next(new restify.errors.UnprocessableEntityError("Match is full"))
      }
      if (req.body.player && req.body.player.id != null && req.resources.match.hasPlayer(req.body.player.id)) {
        return next(new restify.errors.UnprocessableEntityError("A player with the same id is already in the match."))
      }
      ...
      if (builder.hasErrors()) {
        ...
        const wasAdded = req.resources.match.addRequest(builder.request, req.resources.match.game.matchmaking_config);
        if (!wasAdded) {
          return next(new restify.errors.UnprocessableEntityError("Could not add player to match"))
        }
  ...
  server.get('/v1/matches',
    $.restrict('admin player developer playfab'),
    (req, res, next) => {
      if (req.user.role === 'playfab') {
        if (req.query['game.id'] == null) {
          next(new restify.errors.BadRequestError("Need to query on ?game.id= when authenticating with PlayFab"))
        } else {
          req.params.game = req.query['game.id'];
          $.load({game: 'Game'})(req, res, next)
        }
      } else {
        next();
      }
