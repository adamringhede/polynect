...
MatchQueryBuilder.prototype.build = function () {
  ...
  //
  // Get attributes
  if(this.config.attributes && Object.keys(this.config.attributes).length > 0) {
  ...
  if (this.config.roles && this.config.roles.limits && Object.keys(this.config.roles.limits).length > 0) { // assuming that the roles configuration is valid
...
MatchQueryBuilder.prototype.getRolesQuery = function () {
  ...
  var fn = inject(rolesFn, {
    ...
    ALLOW_SWITCHING: this.config.roles.allow_switching == true
  });
...
MatchQueryBuilder.prototype.createRequest = function() {
  ...
  var request = {
    player: {
      id: this.player.player_id || this.player._id || this.player.id
    },
    attributes: {},
    roles: null,
    min: 0,
