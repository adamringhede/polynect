...
const Builder = require('../../Components/MatchQueryBuilder/MatchQueryBuilder');
const async = require('async');
const ObjectId = require('objectid');

const MATCH_RESERVATION_TIMEOUT = 50;

exports.findMatchForGroup = function (requests, game) {
  return new Promise((resolve, reject) => {
    var match = new Models.Match.initWithGame(game);
    for (let data of requests) {
      var builder = new Builder(
        game.matchmaking_config,
        data.values, // Input values
        data.player, // Player
        data.character || null, // Character
        'attributes'
      );
    ...
    match.save((err) => {
      ...
      if (match.max_teams > 1) {
        match.matchWithTeams().then(() => resolve(match));
      } else {
        resolve(match);
      }
    });
...
exports.findMatchForMatch = function (match) {
  return new Promise((resolve, reject) => {
...
exports.findMatch = function(params, callback) {
  ...
  findMatch(game, player.player_id || player._id, attempts, builder, function (err, match) {
    ...
    match.save(function (err) {
      if (match.max_teams > 1) {
        match.matchWithTeams().then(() => callback(err, match));
      } else {
        callback(err, match);
      }
    });
...
function findMatch(game, player_id, attempts, builder, callback, attempt) {
  ...
  var roles = builder.config.roles && builder.config.roles.limits && Object.keys(builder.config.roles.limits).length > 0 ? builder.getPlayerRoles() : false;
